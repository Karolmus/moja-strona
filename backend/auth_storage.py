import os
import secrets
import sqlite3
import hashlib
import math
from base64 import urlsafe_b64encode
from datetime import datetime, timedelta, timezone
from zoneinfo import ZoneInfo

from cryptography.fernet import Fernet, InvalidToken
from flask import g
from werkzeug.security import check_password_hash, generate_password_hash

try:
    import psycopg
    from psycopg.rows import dict_row
except ImportError:
    psycopg = None
    dict_row = None

INTEGRITY_ERRORS = (sqlite3.IntegrityError,)

if psycopg is not None:
    INTEGRITY_ERRORS = INTEGRITY_ERRORS + (psycopg.IntegrityError,)


class ScheduleReservationConflict(Exception):
    """A requested timetable slot has already been reserved."""


_last_analytics_cleanup_day = None

DEFAULT_SITE_PRICES = {
    "ongoing": 100,
    "exam8": 100,
    "matura_basic": 100,
    "matura_extended": 130,
}

ANALYTICS_RESET_KEY = "analytics_reset_2026_08_05_tabula_rasa"


def database_path():
    configured_path = os.environ.get("DATABASE_PATH")

    if configured_path:
        return configured_path

    return os.path.join(os.path.dirname(__file__), "instance", "deltasigma.sqlite3")


def database_url():
    return os.environ.get("DATABASE_URL")


def database_engine():
    return "postgres" if database_url() else "sqlite"


def now_iso():
    return datetime.now(timezone.utc).isoformat()


def credential_cipher():
    secret = (
        os.environ.get("CREDENTIAL_ENCRYPTION_KEY", "").strip()
        or os.environ.get("SECRET_KEY", "").strip()
    )

    if not secret:
        if os.environ.get("RENDER"):
            raise RuntimeError("Ustaw SECRET_KEY lub CREDENTIAL_ENCRYPTION_KEY w usłudze Render.")

        secret = "delta-sigma-local-development-credential-vault"

    key_material = hashlib.sha256(
        f"{secret}:delta-sigma-credential-vault-v1".encode("utf-8")
    ).digest()

    return Fernet(urlsafe_b64encode(key_material))


def encrypt_credential(value):
    if value in (None, ""):
        return None

    return credential_cipher().encrypt(str(value).encode("utf-8")).decode("utf-8")


def decrypt_credential(value):
    if not value:
        return ""

    try:
        return credential_cipher().decrypt(str(value).encode("utf-8")).decode("utf-8")
    except (InvalidToken, TypeError, UnicodeDecodeError, ValueError):
        return ""


def parent_access_expiry():
    now = datetime.now(timezone.utc).astimezone(ZoneInfo("Europe/Warsaw"))
    end_year = now.year + 1 if (now.month, now.day) >= (8, 1) else now.year
    end_of_school_year = datetime(
        end_year,
        8,
        31,
        23,
        59,
        59,
        tzinfo=ZoneInfo("Europe/Warsaw"),
    )

    return end_of_school_year.astimezone(timezone.utc).isoformat()


def analytics_today():
    try:
        return datetime.now(ZoneInfo("Europe/Warsaw")).date()
    except Exception:
        return datetime.now(timezone.utc).date()


def normalize_email(email):
    return str(email or "").strip().lower()


def generate_temporary_password(length=14):
    return secrets.token_urlsafe(length)[:length]


def generate_parent_access_token():
    return secrets.token_urlsafe(32)


def hash_parent_access_token(token):
    return hashlib.sha256(str(token or "").encode("utf-8")).hexdigest()


def db_bool(value):
    if database_engine() == "postgres":
        return bool(value)

    return 1 if bool(value) else 0


def duration_seconds(value):
    try:
        seconds = int(value or 0)
    except (TypeError, ValueError):
        seconds = 0

    return max(0, min(seconds, 4 * 60 * 60))


def bounded_int(value, minimum=0, maximum=100000):
    try:
        number = int(value or 0)
    except (TypeError, ValueError):
        number = minimum

    return max(minimum, min(number, maximum))


def bounded_float(value, minimum=0.0, maximum=100000.0):
    try:
        number = float(value or 0)
    except (TypeError, ValueError):
        number = minimum

    return max(minimum, min(number, maximum))


def point_value(value):
    if value is None or value == "":
        return None

    try:
        points = float(value)
    except (TypeError, ValueError):
        return None

    if not math.isfinite(points):
        return None

    if points < 0:
        return 0.0

    return round(min(points, 10000), 2)


def bounded_text(value, maximum_length, default=None):
    if value is None:
        return default

    text = str(value).strip()

    if not text:
        return default

    return text[:maximum_length]


def get_db():
    if "db" not in g:
        if database_engine() == "postgres":
            if psycopg is None:
                raise RuntimeError("Brakuje pakietu psycopg wymaganego do połączenia z Neon Postgres.")

            g.db = psycopg.connect(database_url(), row_factory=dict_row)
        else:
            path = database_path()
            os.makedirs(os.path.dirname(path), exist_ok=True)

            connection = sqlite3.connect(path)
            connection.row_factory = sqlite3.Row
            g.db = connection

    return g.db


def prepare_sql(sql):
    if database_engine() == "postgres":
        return sql.replace("?", "%s")

    return sql


def execute(sql, values=()):
    return get_db().execute(prepare_sql(sql), values)


def close_db(_error=None):
    connection = g.pop("db", None)

    if connection is not None:
        connection.close()


def register_auth_db(app):
    app.teardown_appcontext(close_db)


def apply_one_time_analytics_reset(db):
    timestamp = now_iso()

    if database_engine() == "postgres":
        inserted = db.execute(
            """
            INSERT INTO site_settings (key, value, updated_at)
            VALUES (%s, %s, %s)
            ON CONFLICT (key) DO NOTHING
            RETURNING key
            """,
            (ANALYTICS_RESET_KEY, timestamp, timestamp),
        ).fetchone()
        should_reset = inserted is not None
    else:
        result = db.execute(
            """
            INSERT OR IGNORE INTO site_settings (key, value, updated_at)
            VALUES (?, ?, ?)
            """,
            (ANALYTICS_RESET_KEY, timestamp, timestamp),
        )
        should_reset = result.rowcount == 1

    if not should_reset:
        return False

    for table in (
        "site_analytics_campaigns",
        "site_analytics_sessions",
        "site_analytics_visitors",
        "site_analytics_daily",
    ):
        db.execute(f"DELETE FROM {table}")

    return True


def row_to_dict(row):
    if row is None:
        return None

    return dict(row)


def review_item_to_dict(row):
    item = row_to_dict(row)

    if item:
        item["is_resolved"] = bool(item["is_resolved"])

    return item


def init_auth_db():
    db = get_db()

    if database_engine() == "postgres":
        statements = [
            """
            CREATE TABLE IF NOT EXISTS users (
                id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                email TEXT NOT NULL UNIQUE,
                display_name TEXT NOT NULL,
                role TEXT NOT NULL CHECK(role IN ('admin', 'student')),
                level TEXT,
                password_hash TEXT NOT NULL,
                password_ciphertext TEXT,
                auth_version INTEGER NOT NULL DEFAULT 1,
                is_active BOOLEAN NOT NULL DEFAULT TRUE,
                created_at TEXT NOT NULL,
                last_login_at TEXT
            )
            """,
            """
            CREATE TABLE IF NOT EXISTS task_progress (
                id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
                task_id TEXT NOT NULL,
                source_id TEXT,
                file TEXT,
                level TEXT,
                topic TEXT,
                difficulty INTEGER,
                result TEXT NOT NULL CHECK(result IN ('good', 'medium', 'bad')),
                hint_used BOOLEAN NOT NULL DEFAULT FALSE,
                answer_shown BOOLEAN NOT NULL DEFAULT FALSE,
                task_mode TEXT,
                duration_seconds INTEGER NOT NULL DEFAULT 0,
                earned_points DOUBLE PRECISION,
                max_points DOUBLE PRECISION,
                created_at TEXT NOT NULL
            )
            """,
            """
            CREATE INDEX IF NOT EXISTS idx_task_progress_user_id
                ON task_progress(user_id)
            """,
            """
            CREATE INDEX IF NOT EXISTS idx_task_progress_task_id
                ON task_progress(task_id)
            """,
            """
            CREATE INDEX IF NOT EXISTS idx_task_progress_user_created_at
                ON task_progress(user_id, created_at DESC)
            """,
            """
            CREATE TABLE IF NOT EXISTS speed_training_results (
                id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
                level TEXT NOT NULL,
                topic TEXT NOT NULL,
                difficulty TEXT NOT NULL,
                round_seconds INTEGER NOT NULL,
                correct_count INTEGER NOT NULL,
                mistake_count INTEGER NOT NULL,
                best_streak INTEGER NOT NULL,
                average_time_seconds DOUBLE PRECISION NOT NULL DEFAULT 0,
                created_at TEXT NOT NULL
            )
            """,
            """
            CREATE INDEX IF NOT EXISTS idx_speed_training_results_user_id
                ON speed_training_results(user_id)
            """,
            """
            CREATE INDEX IF NOT EXISTS idx_speed_training_results_scope
                ON speed_training_results(level, topic, difficulty, round_seconds)
            """,
            """
            CREATE INDEX IF NOT EXISTS idx_speed_training_results_user_created_at
                ON speed_training_results(user_id, created_at DESC)
            """,
            """
            CREATE INDEX IF NOT EXISTS idx_speed_training_results_ranking
                ON speed_training_results(
                    level,
                    topic,
                    difficulty,
                    round_seconds,
                    created_at DESC,
                    user_id
                )
            """,
            """
            CREATE TABLE IF NOT EXISTS speed_training_attempts (
                token_hash TEXT PRIMARY KEY,
                user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
                started_at TEXT NOT NULL,
                expires_at TEXT NOT NULL,
                used_at TEXT
            )
            """,
            """
            CREATE TABLE IF NOT EXISTS parent_access_tokens (
                id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
                token_hash TEXT NOT NULL UNIQUE,
                token_ciphertext TEXT,
                label TEXT,
                is_active BOOLEAN NOT NULL DEFAULT TRUE,
                created_at TEXT NOT NULL,
                expires_at TEXT NOT NULL,
                last_used_at TEXT,
                revoked_at TEXT
            )
            """,
            """
            CREATE INDEX IF NOT EXISTS idx_parent_access_tokens_user_id
                ON parent_access_tokens(user_id)
            """,
            """
            CREATE INDEX IF NOT EXISTS idx_parent_access_tokens_token_hash
                ON parent_access_tokens(token_hash)
            """,
            """
            CREATE TABLE IF NOT EXISTS task_review_items (
                id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
                task_id TEXT NOT NULL,
                source_id TEXT,
                file TEXT,
                level TEXT,
                topic TEXT,
                difficulty INTEGER,
                title TEXT,
                course_part TEXT,
                note TEXT,
                is_resolved BOOLEAN NOT NULL DEFAULT FALSE,
                created_at TEXT NOT NULL,
                resolved_at TEXT
            )
            """,
            """
            CREATE INDEX IF NOT EXISTS idx_task_review_items_user_id
                ON task_review_items(user_id)
            """,
            """
            CREATE INDEX IF NOT EXISTS idx_task_review_items_task_id
                ON task_review_items(task_id)
            """,
            """
            CREATE TABLE IF NOT EXISTS contact_messages (
                id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                contact TEXT NOT NULL,
                preferred_term TEXT,
                message TEXT NOT NULL,
                origin TEXT NOT NULL DEFAULT 'prospect',
                user_id INTEGER REFERENCES users(id) ON DELETE SET NULL,
                is_read BOOLEAN NOT NULL DEFAULT FALSE,
                created_at TEXT NOT NULL,
                read_at TEXT,
                deleted_at TEXT
            )
            """,
            """
            CREATE INDEX IF NOT EXISTS idx_contact_messages_created_at
                ON contact_messages(created_at)
            """,
            """
            CREATE TABLE IF NOT EXISTS schedule_reservations (
                term_key TEXT PRIMARY KEY,
                message_id INTEGER NOT NULL REFERENCES contact_messages(id) ON DELETE CASCADE,
                term_label TEXT NOT NULL,
                created_at TEXT NOT NULL
            )
            """,
            """
            CREATE INDEX IF NOT EXISTS idx_schedule_reservations_message_id
                ON schedule_reservations(message_id)
            """,
            """
            CREATE TABLE IF NOT EXISTS site_analytics_daily (
                day TEXT NOT NULL,
                path TEXT NOT NULL,
                referrer_host TEXT NOT NULL DEFAULT '',
                device_type TEXT NOT NULL,
                page_views INTEGER NOT NULL DEFAULT 0,
                PRIMARY KEY(day, path, referrer_host, device_type)
            )
            """,
            """
            CREATE INDEX IF NOT EXISTS idx_site_analytics_daily_day
                ON site_analytics_daily(day)
            """,
            """
            CREATE TABLE IF NOT EXISTS site_analytics_visitors (
                day TEXT NOT NULL,
                visitor_hash TEXT NOT NULL,
                path TEXT NOT NULL,
                PRIMARY KEY(day, visitor_hash, path)
            )
            """,
            """
            CREATE INDEX IF NOT EXISTS idx_site_analytics_visitors_day
                ON site_analytics_visitors(day)
            """,
            """
            CREATE TABLE IF NOT EXISTS site_analytics_sessions (
                day TEXT NOT NULL,
                session_hash TEXT NOT NULL,
                path TEXT NOT NULL,
                PRIMARY KEY(day, session_hash, path)
            )
            """,
            """
            CREATE INDEX IF NOT EXISTS idx_site_analytics_sessions_day
                ON site_analytics_sessions(day)
            """,
            """
            CREATE TABLE IF NOT EXISTS site_analytics_campaigns (
                day TEXT NOT NULL,
                session_hash TEXT NOT NULL,
                source TEXT NOT NULL,
                medium TEXT NOT NULL DEFAULT '',
                campaign TEXT NOT NULL DEFAULT '',
                content TEXT NOT NULL DEFAULT '',
                landing_path TEXT NOT NULL,
                PRIMARY KEY(day, session_hash, source, medium, campaign, content, landing_path)
            )
            """,
            """
            CREATE INDEX IF NOT EXISTS idx_site_analytics_campaigns_day
                ON site_analytics_campaigns(day)
            """,
            """
            CREATE TABLE IF NOT EXISTS site_settings (
                key TEXT PRIMARY KEY,
                value TEXT NOT NULL,
                updated_at TEXT NOT NULL
            )
            """,
            """
            CREATE TABLE IF NOT EXISTS security_rate_limits (
                scope TEXT NOT NULL,
                key_hash TEXT NOT NULL,
                window_started_at TEXT NOT NULL,
                request_count INTEGER NOT NULL DEFAULT 0,
                PRIMARY KEY(scope, key_hash)
            )
            """,
            """
            ALTER TABLE task_progress
                ADD COLUMN IF NOT EXISTS duration_seconds INTEGER NOT NULL DEFAULT 0
            """,
            """
            ALTER TABLE task_progress
                ADD COLUMN IF NOT EXISTS earned_points DOUBLE PRECISION
            """,
            """
            ALTER TABLE task_progress
                ADD COLUMN IF NOT EXISTS max_points DOUBLE PRECISION
            """,
            """
            ALTER TABLE contact_messages
                ADD COLUMN IF NOT EXISTS deleted_at TEXT
            """,
            """
            ALTER TABLE contact_messages
                ADD COLUMN IF NOT EXISTS origin TEXT NOT NULL DEFAULT 'prospect'
            """,
            """
            ALTER TABLE contact_messages
                ADD COLUMN IF NOT EXISTS user_id INTEGER REFERENCES users(id) ON DELETE SET NULL
            """,
            """
            ALTER TABLE users
                ADD COLUMN IF NOT EXISTS auth_version INTEGER NOT NULL DEFAULT 1
            """,
            """
            ALTER TABLE users
                ADD COLUMN IF NOT EXISTS password_ciphertext TEXT
            """,
            """
            ALTER TABLE parent_access_tokens
                ADD COLUMN IF NOT EXISTS expires_at TEXT
            """,
            """
            ALTER TABLE parent_access_tokens
                ADD COLUMN IF NOT EXISTS token_ciphertext TEXT
            """,
            """
            CREATE INDEX IF NOT EXISTS idx_contact_messages_deleted_at
                ON contact_messages(deleted_at)
            """,
            """
            CREATE INDEX IF NOT EXISTS idx_contact_messages_origin
                ON contact_messages(origin, deleted_at, created_at DESC)
            """,
        ]

        for statement in statements:
            db.execute(statement)
    else:
        db.executescript(
            """
        CREATE TABLE IF NOT EXISTS users (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            email TEXT NOT NULL UNIQUE,
            display_name TEXT NOT NULL,
            role TEXT NOT NULL CHECK(role IN ('admin', 'student')),
            level TEXT,
            password_hash TEXT NOT NULL,
            password_ciphertext TEXT,
            auth_version INTEGER NOT NULL DEFAULT 1,
            is_active INTEGER NOT NULL DEFAULT 1,
            created_at TEXT NOT NULL,
            last_login_at TEXT
        );

        CREATE TABLE IF NOT EXISTS task_progress (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id INTEGER NOT NULL,
            task_id TEXT NOT NULL,
            source_id TEXT,
            file TEXT,
            level TEXT,
            topic TEXT,
            difficulty INTEGER,
            result TEXT NOT NULL CHECK(result IN ('good', 'medium', 'bad')),
            hint_used INTEGER NOT NULL DEFAULT 0,
            answer_shown INTEGER NOT NULL DEFAULT 0,
            task_mode TEXT,
            duration_seconds INTEGER NOT NULL DEFAULT 0,
            earned_points REAL,
            max_points REAL,
            created_at TEXT NOT NULL,
            FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE
        );

        CREATE INDEX IF NOT EXISTS idx_task_progress_user_id
            ON task_progress(user_id);

        CREATE INDEX IF NOT EXISTS idx_task_progress_task_id
            ON task_progress(task_id);

        CREATE INDEX IF NOT EXISTS idx_task_progress_user_created_at
            ON task_progress(user_id, created_at DESC);

        CREATE TABLE IF NOT EXISTS speed_training_results (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id INTEGER NOT NULL,
            level TEXT NOT NULL,
            topic TEXT NOT NULL,
            difficulty TEXT NOT NULL,
            round_seconds INTEGER NOT NULL,
            correct_count INTEGER NOT NULL,
            mistake_count INTEGER NOT NULL,
            best_streak INTEGER NOT NULL,
            average_time_seconds REAL NOT NULL DEFAULT 0,
            created_at TEXT NOT NULL,
            FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE
        );

        CREATE INDEX IF NOT EXISTS idx_speed_training_results_user_id
            ON speed_training_results(user_id);

        CREATE INDEX IF NOT EXISTS idx_speed_training_results_scope
            ON speed_training_results(level, topic, difficulty, round_seconds);

        CREATE INDEX IF NOT EXISTS idx_speed_training_results_user_created_at
            ON speed_training_results(user_id, created_at DESC);

        CREATE INDEX IF NOT EXISTS idx_speed_training_results_ranking
            ON speed_training_results(
                level,
                topic,
                difficulty,
                round_seconds,
                created_at DESC,
                user_id
            );

        CREATE TABLE IF NOT EXISTS speed_training_attempts (
            token_hash TEXT PRIMARY KEY,
            user_id INTEGER NOT NULL,
            started_at TEXT NOT NULL,
            expires_at TEXT NOT NULL,
            used_at TEXT,
            FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE
        );

        CREATE TABLE IF NOT EXISTS parent_access_tokens (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id INTEGER NOT NULL,
            token_hash TEXT NOT NULL UNIQUE,
            token_ciphertext TEXT,
            label TEXT,
            is_active INTEGER NOT NULL DEFAULT 1,
            created_at TEXT NOT NULL,
            expires_at TEXT,
            last_used_at TEXT,
            revoked_at TEXT,
            FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE
        );

        CREATE INDEX IF NOT EXISTS idx_parent_access_tokens_user_id
            ON parent_access_tokens(user_id);

        CREATE INDEX IF NOT EXISTS idx_parent_access_tokens_token_hash
            ON parent_access_tokens(token_hash);

        CREATE TABLE IF NOT EXISTS task_review_items (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id INTEGER NOT NULL,
            task_id TEXT NOT NULL,
            source_id TEXT,
            file TEXT,
            level TEXT,
            topic TEXT,
            difficulty INTEGER,
            title TEXT,
            course_part TEXT,
            note TEXT,
            is_resolved INTEGER NOT NULL DEFAULT 0,
            created_at TEXT NOT NULL,
            resolved_at TEXT,
            FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE
        );

        CREATE INDEX IF NOT EXISTS idx_task_review_items_user_id
            ON task_review_items(user_id);

        CREATE INDEX IF NOT EXISTS idx_task_review_items_task_id
            ON task_review_items(task_id);

        CREATE TABLE IF NOT EXISTS contact_messages (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            contact TEXT NOT NULL,
            preferred_term TEXT,
            message TEXT NOT NULL,
            origin TEXT NOT NULL DEFAULT 'prospect',
            user_id INTEGER,
            is_read INTEGER NOT NULL DEFAULT 0,
            created_at TEXT NOT NULL,
            read_at TEXT,
            deleted_at TEXT,
            FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE SET NULL
        );

        CREATE INDEX IF NOT EXISTS idx_contact_messages_created_at
            ON contact_messages(created_at);

        CREATE TABLE IF NOT EXISTS schedule_reservations (
            term_key TEXT PRIMARY KEY,
            message_id INTEGER NOT NULL,
            term_label TEXT NOT NULL,
            created_at TEXT NOT NULL,
            FOREIGN KEY(message_id) REFERENCES contact_messages(id) ON DELETE CASCADE
        );

        CREATE INDEX IF NOT EXISTS idx_schedule_reservations_message_id
            ON schedule_reservations(message_id);

        CREATE TABLE IF NOT EXISTS site_analytics_daily (
            day TEXT NOT NULL,
            path TEXT NOT NULL,
            referrer_host TEXT NOT NULL DEFAULT '',
            device_type TEXT NOT NULL,
            page_views INTEGER NOT NULL DEFAULT 0,
            PRIMARY KEY(day, path, referrer_host, device_type)
        );

        CREATE INDEX IF NOT EXISTS idx_site_analytics_daily_day
            ON site_analytics_daily(day);

        CREATE TABLE IF NOT EXISTS site_analytics_visitors (
            day TEXT NOT NULL,
            visitor_hash TEXT NOT NULL,
            path TEXT NOT NULL,
            PRIMARY KEY(day, visitor_hash, path)
        );

        CREATE INDEX IF NOT EXISTS idx_site_analytics_visitors_day
            ON site_analytics_visitors(day);

        CREATE TABLE IF NOT EXISTS site_analytics_sessions (
            day TEXT NOT NULL,
            session_hash TEXT NOT NULL,
            path TEXT NOT NULL,
            PRIMARY KEY(day, session_hash, path)
        );

        CREATE INDEX IF NOT EXISTS idx_site_analytics_sessions_day
            ON site_analytics_sessions(day);

        CREATE TABLE IF NOT EXISTS site_analytics_campaigns (
            day TEXT NOT NULL,
            session_hash TEXT NOT NULL,
            source TEXT NOT NULL,
            medium TEXT NOT NULL DEFAULT '',
            campaign TEXT NOT NULL DEFAULT '',
            content TEXT NOT NULL DEFAULT '',
            landing_path TEXT NOT NULL,
            PRIMARY KEY(day, session_hash, source, medium, campaign, content, landing_path)
        );

        CREATE INDEX IF NOT EXISTS idx_site_analytics_campaigns_day
            ON site_analytics_campaigns(day);

        CREATE TABLE IF NOT EXISTS site_settings (
            key TEXT PRIMARY KEY,
            value TEXT NOT NULL,
            updated_at TEXT NOT NULL
        );

        CREATE TABLE IF NOT EXISTS security_rate_limits (
            scope TEXT NOT NULL,
            key_hash TEXT NOT NULL,
            window_started_at TEXT NOT NULL,
            request_count INTEGER NOT NULL DEFAULT 0,
            PRIMARY KEY(scope, key_hash)
        );
        """
        )

        columns = {
            row["name"]
            for row in db.execute("PRAGMA table_info(task_progress)").fetchall()
        }

        if "duration_seconds" not in columns:
            db.execute("ALTER TABLE task_progress ADD COLUMN duration_seconds INTEGER NOT NULL DEFAULT 0")

        if "earned_points" not in columns:
            db.execute("ALTER TABLE task_progress ADD COLUMN earned_points REAL")

        if "max_points" not in columns:
            db.execute("ALTER TABLE task_progress ADD COLUMN max_points REAL")

        contact_columns = {
            row["name"]
            for row in db.execute("PRAGMA table_info(contact_messages)").fetchall()
        }

        if "deleted_at" not in contact_columns:
            db.execute("ALTER TABLE contact_messages ADD COLUMN deleted_at TEXT")

        if "origin" not in contact_columns:
            db.execute("ALTER TABLE contact_messages ADD COLUMN origin TEXT NOT NULL DEFAULT 'prospect'")

        if "user_id" not in contact_columns:
            db.execute("ALTER TABLE contact_messages ADD COLUMN user_id INTEGER")

        user_columns = {
            row["name"]
            for row in db.execute("PRAGMA table_info(users)").fetchall()
        }

        if "auth_version" not in user_columns:
            db.execute("ALTER TABLE users ADD COLUMN auth_version INTEGER NOT NULL DEFAULT 1")

        if "password_ciphertext" not in user_columns:
            db.execute("ALTER TABLE users ADD COLUMN password_ciphertext TEXT")

        parent_access_columns = {
            row["name"]
            for row in db.execute("PRAGMA table_info(parent_access_tokens)").fetchall()
        }

        if "expires_at" not in parent_access_columns:
            db.execute("ALTER TABLE parent_access_tokens ADD COLUMN expires_at TEXT")

        if "token_ciphertext" not in parent_access_columns:
            db.execute("ALTER TABLE parent_access_tokens ADD COLUMN token_ciphertext TEXT")

        db.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_contact_messages_deleted_at
                ON contact_messages(deleted_at)
            """
        )
        db.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_contact_messages_origin
                ON contact_messages(origin, deleted_at, created_at DESC)
            """
        )

    school_year_expiry = parent_access_expiry()
    db.execute(
        prepare_sql(
            """
            UPDATE parent_access_tokens
            SET expires_at = ?
            WHERE is_active = ?
                AND (
                    expires_at IS NULL
                    OR (expires_at > ? AND expires_at < ?)
                )
            """
        ),
        (school_year_expiry, db_bool(True), now_iso(), school_year_expiry),
    )

    apply_one_time_analytics_reset(db)
    db.commit()


def sanitize_site_prices(values):
    prices = dict(DEFAULT_SITE_PRICES)

    if isinstance(values, dict):
        for key in DEFAULT_SITE_PRICES:
            if key not in values:
                continue

            try:
                value = int(values[key])
            except (TypeError, ValueError):
                value = DEFAULT_SITE_PRICES[key]

            prices[key] = max(0, min(value, 10000))

    return prices


def consume_security_rate_limit(scope, key, limit, window_seconds):
    now = datetime.now(timezone.utc)
    cutoff = now - timedelta(seconds=max(1, int(window_seconds)))
    key_hash = hashlib.sha256(str(key or "unknown").encode("utf-8")).hexdigest()
    db = get_db()
    row = db.execute(
        prepare_sql(
            """
            INSERT INTO security_rate_limits (
                scope,
                key_hash,
                window_started_at,
                request_count
            )
            VALUES (?, ?, ?, 1)
            ON CONFLICT(scope, key_hash) DO UPDATE SET
                request_count = CASE
                    WHEN security_rate_limits.window_started_at <= ? THEN 1
                    ELSE security_rate_limits.request_count + 1
                END,
                window_started_at = CASE
                    WHEN security_rate_limits.window_started_at <= ? THEN excluded.window_started_at
                    ELSE security_rate_limits.window_started_at
                END
            RETURNING window_started_at, request_count
            """
        ),
        (
            str(scope)[:80],
            key_hash,
            now.isoformat(),
            cutoff.isoformat(),
            cutoff.isoformat(),
        ),
    ).fetchone()
    db.commit()

    count = int(row["request_count"] or 0)

    if count <= int(limit):
        return True, 0

    try:
        window_started_at = datetime.fromisoformat(row["window_started_at"])
        retry_after = math.ceil(
            max(1, window_seconds - (now - window_started_at).total_seconds())
        )
    except (TypeError, ValueError):
        retry_after = int(window_seconds)

    return False, retry_after


def get_site_prices():
    rows = execute(
        """
        SELECT key, value
        FROM site_settings
        WHERE key IN ('price_ongoing', 'price_exam8', 'price_matura_basic', 'price_matura_extended')
        """
    ).fetchall()
    values = {
        str(row["key"]).replace("price_", "", 1): row["value"]
        for row in rows
    }

    return sanitize_site_prices(values)


def update_site_prices(values):
    prices = sanitize_site_prices(values)
    db = get_db()
    updated_at = now_iso()

    if database_engine() == "postgres":
        for key, value in prices.items():
            db.execute(
                """
                INSERT INTO site_settings (key, value, updated_at)
                VALUES (%s, %s, %s)
                ON CONFLICT (key) DO UPDATE
                    SET value = EXCLUDED.value,
                        updated_at = EXCLUDED.updated_at
                """,
                (f"price_{key}", str(value), updated_at),
            )
    else:
        for key, value in prices.items():
            db.execute(
                """
                INSERT INTO site_settings (key, value, updated_at)
                VALUES (?, ?, ?)
                ON CONFLICT(key) DO UPDATE SET
                    value = excluded.value,
                    updated_at = excluded.updated_at
                """,
                (f"price_{key}", str(value), updated_at),
            )

    db.commit()

    return get_site_prices()


def record_site_pageview(
    path,
    visitor_hash,
    session_hash,
    referrer_host="",
    device_type="desktop",
    campaign=None,
):
    global _last_analytics_cleanup_day

    day = analytics_today()
    day_text = day.isoformat()
    db = get_db()

    db.execute(
        prepare_sql(
            """
            INSERT INTO site_analytics_daily (
                day,
                path,
                referrer_host,
                device_type,
                page_views
            )
            VALUES (?, ?, ?, ?, 1)
            ON CONFLICT(day, path, referrer_host, device_type)
            DO UPDATE SET page_views = site_analytics_daily.page_views + 1
            """
        ),
        (day_text, path, referrer_host, device_type),
    )
    db.execute(
        prepare_sql(
            """
            INSERT INTO site_analytics_visitors (day, visitor_hash, path)
            VALUES (?, ?, ?)
            ON CONFLICT(day, visitor_hash, path) DO NOTHING
            """
        ),
        (day_text, visitor_hash, path),
    )
    db.execute(
        prepare_sql(
            """
            INSERT INTO site_analytics_sessions (day, session_hash, path)
            VALUES (?, ?, ?)
            ON CONFLICT(day, session_hash, path) DO NOTHING
            """
        ),
        (day_text, session_hash, path),
    )

    campaign = campaign or {}

    if campaign.get("source"):
        db.execute(
            prepare_sql(
                """
                INSERT INTO site_analytics_campaigns (
                    day,
                    session_hash,
                    source,
                    medium,
                    campaign,
                    content,
                    landing_path
                )
                VALUES (?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(
                    day,
                    session_hash,
                    source,
                    medium,
                    campaign,
                    content,
                    landing_path
                ) DO NOTHING
                """
            ),
            (
                day_text,
                session_hash,
                campaign["source"],
                campaign.get("medium", ""),
                campaign.get("campaign", ""),
                campaign.get("content", ""),
                campaign.get("landing_path", path),
            ),
        )

    if _last_analytics_cleanup_day != day_text:
        retention_days = bounded_int(
            os.environ.get("ANALYTICS_RETENTION_DAYS", "180"),
            minimum=30,
            maximum=730,
        )
        cutoff = (day - timedelta(days=retention_days)).isoformat()

        for table in (
            "site_analytics_daily",
            "site_analytics_visitors",
            "site_analytics_sessions",
            "site_analytics_campaigns",
        ):
            db.execute(
                prepare_sql(f"DELETE FROM {table} WHERE day < ?"),
                (cutoff,),
            )

        _last_analytics_cleanup_day = day_text

    db.commit()


def site_analytics_summary(days=30):
    period_days = max(1, min(int(days or 30), 90))
    end_day = analytics_today()
    start_day = end_day - timedelta(days=period_days - 1)
    start_text = start_day.isoformat()

    view_rows = execute(
        """
        SELECT day, SUM(page_views) AS total
        FROM site_analytics_daily
        WHERE day >= ?
        GROUP BY day
        ORDER BY day
        """,
        (start_text,),
    ).fetchall()
    visitor_rows = execute(
        """
        SELECT day, COUNT(DISTINCT visitor_hash) AS total
        FROM site_analytics_visitors
        WHERE day >= ?
        GROUP BY day
        ORDER BY day
        """,
        (start_text,),
    ).fetchall()
    session_rows = execute(
        """
        SELECT day, COUNT(DISTINCT session_hash) AS total
        FROM site_analytics_sessions
        WHERE day >= ?
        GROUP BY day
        ORDER BY day
        """,
        (start_text,),
    ).fetchall()

    view_by_day = {row["day"]: int(row["total"] or 0) for row in view_rows}
    visitors_by_day = {row["day"]: int(row["total"] or 0) for row in visitor_rows}
    sessions_by_day = {row["day"]: int(row["total"] or 0) for row in session_rows}
    daily = []

    for offset in range(period_days):
        day_text = (start_day + timedelta(days=offset)).isoformat()
        daily.append({
            "day": day_text,
            "views": view_by_day.get(day_text, 0),
            "visitors": visitors_by_day.get(day_text, 0),
            "sessions": sessions_by_day.get(day_text, 0),
        })

    total_views_row = execute(
        "SELECT SUM(page_views) AS total FROM site_analytics_daily WHERE day >= ?",
        (start_text,),
    ).fetchone()
    total_visitors_row = execute(
        "SELECT COUNT(DISTINCT visitor_hash) AS total FROM site_analytics_visitors WHERE day >= ?",
        (start_text,),
    ).fetchone()
    total_sessions_row = execute(
        "SELECT COUNT(DISTINCT session_hash) AS total FROM site_analytics_sessions WHERE day >= ?",
        (start_text,),
    ).fetchone()
    total_views = int(total_views_row["total"] or 0)
    total_visitors = int(total_visitors_row["total"] or 0)
    total_sessions = int(total_sessions_row["total"] or 0)

    page_rows = execute(
        """
        SELECT path, SUM(page_views) AS views
        FROM site_analytics_daily
        WHERE day >= ?
        GROUP BY path
        ORDER BY views DESC, path
        LIMIT 12
        """,
        (start_text,),
    ).fetchall()
    page_visitor_rows = execute(
        """
        SELECT path, COUNT(DISTINCT visitor_hash) AS visitors
        FROM site_analytics_visitors
        WHERE day >= ?
        GROUP BY path
        """,
        (start_text,),
    ).fetchall()
    page_visitors = {
        row["path"]: int(row["visitors"] or 0)
        for row in page_visitor_rows
    }

    referrer_rows = execute(
        """
        SELECT referrer_host, SUM(page_views) AS views
        FROM site_analytics_daily
        WHERE day >= ? AND referrer_host <> '__internal__'
        GROUP BY referrer_host
        ORDER BY views DESC, referrer_host
        LIMIT 10
        """,
        (start_text,),
    ).fetchall()
    device_rows = execute(
        """
        SELECT device_type, SUM(page_views) AS views
        FROM site_analytics_daily
        WHERE day >= ?
        GROUP BY device_type
        ORDER BY views DESC, device_type
        """,
        (start_text,),
    ).fetchall()
    campaign_rows = execute(
        """
        SELECT
            source,
            medium,
            campaign,
            content,
            landing_path,
            COUNT(*) AS clicks
        FROM site_analytics_campaigns
        WHERE day >= ?
        GROUP BY source, medium, campaign, content, landing_path
        ORDER BY clicks DESC, source, medium, content
        LIMIT 30
        """,
        (start_text,),
    ).fetchall()

    return {
        "days": period_days,
        "range_start": start_text,
        "range_end": end_day.isoformat(),
        "overview": {
            "views": total_views,
            "visitors": total_visitors,
            "sessions": total_sessions,
            "views_per_session": round(total_views / total_sessions, 1) if total_sessions else 0,
        },
        "daily": daily,
        "pages": [
            {
                "path": row["path"],
                "views": int(row["views"] or 0),
                "visitors": page_visitors.get(row["path"], 0),
            }
            for row in page_rows
        ],
        "referrers": [
            {
                "host": row["referrer_host"] or "",
                "views": int(row["views"] or 0),
            }
            for row in referrer_rows
        ],
        "devices": [
            {
                "type": row["device_type"],
                "views": int(row["views"] or 0),
            }
            for row in device_rows
        ],
        "campaigns": [
            {
                "source": row["source"],
                "medium": row["medium"],
                "campaign": row["campaign"],
                "content": row["content"],
                "landing_path": row["landing_path"],
                "clicks": int(row["clicks"] or 0),
            }
            for row in campaign_rows
        ],
    }


def public_user(row):
    user = row_to_dict(row)

    if not user:
        return None

    user.pop("password_hash", None)
    user.pop("password_ciphertext", None)
    user.pop("auth_version", None)
    user["is_active"] = bool(user["is_active"])

    return user


def parent_access_to_dict(row):
    item = row_to_dict(row)

    if item:
        item.pop("token_hash", None)
        item.pop("token_ciphertext", None)
        item["is_active"] = bool(item["is_active"])

    return item


def get_user_by_email(email):
    return row_to_dict(execute("SELECT * FROM users WHERE email = ?", (normalize_email(email),)).fetchone())


def get_user_by_id(user_id):
    return row_to_dict(execute("SELECT * FROM users WHERE id = ?", (user_id,)).fetchone())


def create_user(email, display_name, password, role="student", level="matura_podstawowa", is_active=True):
    normalized_email = normalize_email(email)
    created_at = now_iso()
    password_ciphertext = encrypt_credential(password) if role == "student" else None
    db = get_db()

    db.execute(
        prepare_sql(
            """
            INSERT INTO users (
                email,
                display_name,
                role,
                level,
                password_hash,
                password_ciphertext,
                is_active,
                created_at
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """
        ),
        (
            normalized_email,
            str(display_name or normalized_email).strip(),
            role,
            level,
            generate_password_hash(password),
            password_ciphertext,
            db_bool(is_active),
            created_at,
        ),
    )
    db.commit()

    return get_user_by_email(normalized_email)


def ensure_admin_user(email, password):
    existing = get_user_by_email(email)

    if existing:
        return existing, False

    return create_user(
        email=email,
        display_name="Administrator",
        password=password,
        role="admin",
        level=None,
        is_active=True,
    ), True


def sync_admin_user(email, password=None):
    existing = get_user_by_email(email)

    if not existing:
        return ensure_admin_user(email, password or generate_temporary_password())

    assignments = [
        "display_name = ?",
        "role = ?",
        "level = ?",
        "is_active = ?",
    ]
    values = [
        existing.get("display_name") or "Administrator",
        "admin",
        None,
        db_bool(True),
    ]

    if password and not check_password_hash(existing.get("password_hash") or "", password):
        assignments.append("password_hash = ?")
        values.append(generate_password_hash(password))
        assignments.append("auth_version = auth_version + 1")

    values.append(existing["id"])
    db = get_db()
    db.execute(
        prepare_sql(f"UPDATE users SET {', '.join(assignments)} WHERE id = ?"),
        values,
    )
    db.commit()

    return get_user_by_id(existing["id"]), False


def touch_last_login(user_id):
    db = get_db()

    db.execute(
        prepare_sql("UPDATE users SET last_login_at = ? WHERE id = ?"),
        (now_iso(), user_id),
    )
    db.commit()


def list_students():
    rows = execute(
        """
        SELECT
            u.id,
            u.email,
            u.display_name,
            u.role,
            u.level,
            u.is_active,
            u.created_at,
            u.last_login_at,
            u.password_ciphertext,
            EXISTS (
                SELECT 1
                FROM parent_access_tokens parent_access
                WHERE parent_access.user_id = u.id
                    AND parent_access.is_active = ?
                    AND parent_access.expires_at > ?
            ) AS has_parent_access,
            COUNT(p.id) AS attempts,
            COALESCE(SUM(CASE WHEN p.result = 'good' THEN 1 ELSE 0 END), 0) AS good_count,
            COALESCE(SUM(CASE WHEN p.result = 'medium' THEN 1 ELSE 0 END), 0) AS medium_count,
            COALESCE(SUM(CASE WHEN p.result = 'bad' THEN 1 ELSE 0 END), 0) AS bad_count,
            COALESCE(SUM(COALESCE(p.duration_seconds, 0)), 0) AS total_duration_seconds,
            COALESCE(r.review_count, 0) AS review_count,
            MAX(p.created_at) AS last_activity_at
        FROM users u
        LEFT JOIN task_progress p ON p.user_id = u.id
        LEFT JOIN (
            SELECT user_id, COUNT(*) AS review_count
            FROM task_review_items
            WHERE is_resolved = ?
            GROUP BY user_id
        ) r ON r.user_id = u.id
        WHERE u.role = 'student'
        GROUP BY u.id, u.email, u.display_name, u.role, u.level, u.is_active, u.created_at, u.last_login_at, u.password_ciphertext, r.review_count
        ORDER BY u.created_at DESC
        """,
        (db_bool(True), now_iso(), db_bool(False)),
    ).fetchall()

    students = []

    for row in rows:
        student = dict(row)
        student["is_active"] = bool(student["is_active"])
        student["has_stored_password"] = bool(student.pop("password_ciphertext", None))
        student["has_parent_access"] = bool(student["has_parent_access"])
        student["accuracy"] = round((student["good_count"] / student["attempts"]) * 100, 1) if student["attempts"] else 0
        student["average_duration_seconds"] = round(student["total_duration_seconds"] / student["attempts"]) if student["attempts"] else 0
        students.append(student)

    return students


def update_student(user_id, fields):
    allowed = {
        "display_name": "display_name",
        "level": "level",
        "is_active": "is_active",
    }
    assignments = []
    values = []

    for key, column in allowed.items():
        if key not in fields:
            continue

        value = fields[key]

        if key == "is_active":
            value = db_bool(value)

        assignments.append(f"{column} = ?")
        values.append(value)

    if "is_active" in fields:
        assignments.append("auth_version = auth_version + 1")

    if not assignments:
        return row_to_dict(
            execute("SELECT * FROM users WHERE id = ? AND role = 'student'", (user_id,)).fetchone()
        )

    values.append(user_id)
    db = get_db()
    cursor = db.execute(
        prepare_sql(f"UPDATE users SET {', '.join(assignments)} WHERE id = ? AND role = 'student'"),
        values,
    )
    db.commit()

    if cursor.rowcount == 0:
        return None

    return row_to_dict(
        execute("SELECT * FROM users WHERE id = ? AND role = 'student'", (user_id,)).fetchone()
    )


def reset_user_password(user_id, password):
    db = get_db()

    cursor = db.execute(
        prepare_sql(
            """
            UPDATE users
            SET password_hash = ?, password_ciphertext = ?, auth_version = auth_version + 1
            WHERE id = ? AND role = 'student'
            """
        ),
        (generate_password_hash(password), encrypt_credential(password), user_id),
    )
    db.commit()

    if cursor.rowcount == 0:
        return None

    return row_to_dict(
        execute("SELECT * FROM users WHERE id = ? AND role = 'student'", (user_id,)).fetchone()
    )


def revoke_user_auth(user_id):
    db = get_db()
    cursor = db.execute(
        prepare_sql("UPDATE users SET auth_version = auth_version + 1 WHERE id = ?"),
        (user_id,),
    )
    db.commit()

    return cursor.rowcount > 0


def delete_student(user_id):
    existing = row_to_dict(
        execute("SELECT * FROM users WHERE id = ? AND role = 'student'", (user_id,)).fetchone()
    )

    if not existing:
        return None

    db = get_db()
    db.execute(prepare_sql("DELETE FROM parent_access_tokens WHERE user_id = ?"), (user_id,))
    db.execute(prepare_sql("DELETE FROM task_review_items WHERE user_id = ?"), (user_id,))
    db.execute(prepare_sql("DELETE FROM task_progress WHERE user_id = ?"), (user_id,))
    db.execute(prepare_sql("DELETE FROM speed_training_attempts WHERE user_id = ?"), (user_id,))
    db.execute(prepare_sql("DELETE FROM speed_training_results WHERE user_id = ?"), (user_id,))
    db.execute(prepare_sql("DELETE FROM users WHERE id = ? AND role = 'student'"), (user_id,))
    db.commit()

    return existing


def active_parent_access_record(user_id):
    return row_to_dict(
        execute(
            """
            SELECT *
            FROM parent_access_tokens
            WHERE user_id = ?
                AND is_active = ?
                AND expires_at > ?
            ORDER BY created_at ASC, id ASC
            LIMIT 1
            """,
            (user_id, db_bool(True), now_iso()),
        ).fetchone()
    )


def get_student_credentials(user_id):
    student = row_to_dict(
        execute("SELECT * FROM users WHERE id = ? AND role = 'student'", (user_id,)).fetchone()
    )

    if not student:
        return None

    access = active_parent_access_record(user_id)
    parent_token = decrypt_credential(access.get("token_ciphertext")) if access else ""

    return {
        "student": student,
        "password": decrypt_credential(student.get("password_ciphertext")),
        "parent_access": parent_access_to_dict(access) if access else None,
        "parent_access_token": parent_token,
        "parent_access_needs_reissue": bool(access and not parent_token),
    }


def create_parent_access_token(user_id, label="Rodzic", replace=False):
    student = row_to_dict(
        execute("SELECT * FROM users WHERE id = ? AND role = 'student'", (user_id,)).fetchone()
    )

    if not student:
        return None, None

    existing = active_parent_access_record(user_id)

    if existing and not replace:
        return parent_access_to_dict(existing), decrypt_credential(existing.get("token_ciphertext"))

    token = generate_parent_access_token()
    created_at = now_iso()
    expires_at = parent_access_expiry()
    db = get_db()

    db.execute(
        prepare_sql(
            """
            UPDATE parent_access_tokens
            SET is_active = ?, revoked_at = ?
            WHERE user_id = ? AND is_active = ?
            """
        ),
        (db_bool(False), created_at, user_id, db_bool(True)),
    )

    values = (
        user_id,
        hash_parent_access_token(token),
        encrypt_credential(token),
        str(label or "Rodzic").strip()[:120],
        db_bool(True),
        created_at,
        expires_at,
    )

    if database_engine() == "postgres":
        cursor = db.execute(
            prepare_sql(
                """
                INSERT INTO parent_access_tokens (
                    user_id,
                    token_hash,
                    token_ciphertext,
                    label,
                    is_active,
                    created_at,
                    expires_at
                )
                VALUES (?, ?, ?, ?, ?, ?, ?)
                RETURNING *
                """
            ),
            values,
        )
        item = parent_access_to_dict(cursor.fetchone())
    else:
        cursor = db.execute(
            """
            INSERT INTO parent_access_tokens (
                user_id,
                token_hash,
                token_ciphertext,
                label,
                is_active,
                created_at,
                expires_at
            )
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            values,
        )
        item = parent_access_to_dict(
            db.execute(
                "SELECT * FROM parent_access_tokens WHERE id = ?",
                (cursor.lastrowid,),
            ).fetchone()
        )

    db.commit()

    return item, token


def disable_parent_access_token(user_id):
    revoked_at = now_iso()
    db = get_db()
    cursor = db.execute(
        prepare_sql(
            """
            UPDATE parent_access_tokens
            SET is_active = ?, revoked_at = ?
            WHERE user_id = ? AND is_active = ?
            """
        ),
        (db_bool(False), revoked_at, user_id, db_bool(True)),
    )
    db.commit()

    return cursor.rowcount > 0


def parent_access_by_token(token):
    token_hash = hash_parent_access_token(token)
    row = execute(
        """
        SELECT
            p.id AS access_id,
            p.user_id AS access_user_id,
            p.label AS access_label,
            p.is_active AS access_is_active,
            p.created_at AS access_created_at,
            p.expires_at AS access_expires_at,
            p.last_used_at AS access_last_used_at,
            p.revoked_at AS access_revoked_at,
            u.id AS student_id,
            u.email AS student_email,
            u.display_name AS student_display_name,
            u.role AS student_role,
            u.level AS student_level,
            u.is_active AS student_is_active,
            u.created_at AS student_created_at,
            u.last_login_at AS student_last_login_at
        FROM parent_access_tokens p
        JOIN users u ON u.id = p.user_id
        WHERE p.token_hash = ?
            AND p.is_active = ?
            AND p.expires_at > ?
            AND u.role = 'student'
        LIMIT 1
        """,
        (token_hash, db_bool(True), now_iso()),
    ).fetchone()

    if not row:
        return None

    item = dict(row)
    student = {
        "id": item["student_id"],
        "email": item["student_email"],
        "display_name": item["student_display_name"],
        "role": item["student_role"],
        "level": item["student_level"],
        "is_active": bool(item["student_is_active"]),
        "created_at": item["student_created_at"],
        "last_login_at": item["student_last_login_at"],
    }
    access = {
        "id": item["access_id"],
        "user_id": item["access_user_id"],
        "label": item["access_label"],
        "is_active": bool(item["access_is_active"]),
        "created_at": item["access_created_at"],
        "expires_at": item["access_expires_at"],
        "last_used_at": item["access_last_used_at"],
        "revoked_at": item["access_revoked_at"],
    }

    return {
        "access": access,
        "student": student,
    }


def touch_parent_access(access_id):
    db = get_db()
    now = datetime.now(timezone.utc)
    cutoff = (now - timedelta(minutes=15)).isoformat()

    db.execute(
        prepare_sql(
            """
            UPDATE parent_access_tokens
            SET last_used_at = ?
            WHERE id = ?
                AND (last_used_at IS NULL OR last_used_at < ?)
            """
        ),
        (now.isoformat(), access_id, cutoff),
    )
    db.commit()


def record_progress(user_id, data):
    created_at = now_iso()
    db = get_db()
    task_id = bounded_text(data.get("task_id"), 240)

    if not task_id:
        raise ValueError("Brakuje identyfikatora zadania.")

    max_points = point_value(data.get("max_points"))
    earned_points = point_value(data.get("earned_points"))

    if max_points is not None and earned_points is not None:
        earned_points = min(earned_points, max_points)

    values = (
        user_id,
        task_id,
        bounded_text(data.get("source_id"), 160),
        bounded_text(data.get("file"), 300),
        bounded_text(data.get("level"), 40),
        bounded_text(data.get("topic"), 160),
        bounded_int(data.get("difficulty"), 0, 10),
        data["result"],
        db_bool(data.get("hint_used")),
        db_bool(data.get("answer_shown")),
        bounded_text(data.get("task_mode"), 40),
        duration_seconds(data.get("duration_seconds")),
        earned_points,
        max_points,
        created_at,
    )

    if database_engine() == "postgres":
        cursor = db.execute(
            prepare_sql(
                """
                INSERT INTO task_progress (
                    user_id,
                    task_id,
                    source_id,
                    file,
                    level,
                    topic,
                    difficulty,
                    result,
                    hint_used,
                    answer_shown,
                    task_mode,
                    duration_seconds,
                    earned_points,
                    max_points,
                    created_at
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                RETURNING *
                """
            ),
            values,
        )
        progress = row_to_dict(cursor.fetchone())
    else:
        cursor = db.execute(
            """
            INSERT INTO task_progress (
                user_id,
                task_id,
                source_id,
                file,
                level,
                topic,
                difficulty,
                result,
                hint_used,
                answer_shown,
                task_mode,
                duration_seconds,
                earned_points,
                max_points,
                created_at
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            values,
        )
        progress = row_to_dict(
            db.execute(
                "SELECT * FROM task_progress WHERE id = ?",
                (cursor.lastrowid,),
            ).fetchone()
        )

    db.commit()

    return progress


def progress_for_user(user_id, limit=2000):
    return [
        dict(row)
        for row in execute(
            """
            SELECT *
            FROM task_progress
            WHERE user_id = ?
            ORDER BY created_at DESC
            LIMIT ?
            """,
            (user_id, limit),
        )
        .fetchall()
    ]


def speed_training_result_to_dict(row, viewer_user_id=None):
    item = row_to_dict(row)

    if not item:
        return None

    for key in ("id", "user_id", "round_seconds", "correct_count", "mistake_count", "best_streak"):
        if key in item and item[key] is not None:
            item[key] = int(item[key])

    if "average_time_seconds" in item and item["average_time_seconds"] is not None:
        item["average_time_seconds"] = round(float(item["average_time_seconds"]), 3)

    if "student_is_active" in item:
        item["student_is_active"] = bool(item["student_is_active"])

    if viewer_user_id is not None:
        item["is_current_user"] = item.get("user_id") == viewer_user_id

    item.pop("user_id", None)

    return item


def record_speed_training_result(user_id, data):
    created_at = now_iso()
    values = (
        user_id,
        str(data.get("level") or "eo").strip()[:40],
        str(data.get("topic") or "all").strip()[:80],
        str(data.get("difficulty") or "1").strip()[:16],
        duration_seconds(data.get("round_seconds")),
        bounded_int(data.get("correct_count"), 0, 10000),
        bounded_int(data.get("mistake_count"), 0, 10000),
        bounded_int(data.get("best_streak"), 0, 10000),
        round(bounded_float(data.get("average_time_seconds"), 0, 3600), 3),
        created_at,
    )
    db = get_db()

    if database_engine() == "postgres":
        cursor = db.execute(
            prepare_sql(
                """
                INSERT INTO speed_training_results (
                    user_id,
                    level,
                    topic,
                    difficulty,
                    round_seconds,
                    correct_count,
                    mistake_count,
                    best_streak,
                    average_time_seconds,
                    created_at
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                RETURNING *
                """
            ),
            values,
        )
        result = speed_training_result_to_dict(cursor.fetchone())
    else:
        cursor = db.execute(
            """
            INSERT INTO speed_training_results (
                user_id,
                level,
                topic,
                difficulty,
                round_seconds,
                correct_count,
                mistake_count,
                best_streak,
                average_time_seconds,
                created_at
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            values,
        )
        result = speed_training_result_to_dict(
            db.execute(
                "SELECT * FROM speed_training_results WHERE id = ?",
                (cursor.lastrowid,),
            ).fetchone()
        )

    db.commit()

    return result


def register_speed_training_attempt(token, user_id, expires_at):
    token_hash = hashlib.sha256(str(token).encode("utf-8")).hexdigest()
    started_at = now_iso()
    db = get_db()
    db.execute(
        prepare_sql(
            """
            INSERT INTO speed_training_attempts (
                token_hash,
                user_id,
                started_at,
                expires_at,
                used_at
            )
            VALUES (?, ?, ?, ?, NULL)
            """
        ),
        (token_hash, user_id, started_at, expires_at),
    )
    db.commit()


def consume_speed_training_attempt(token, user_id):
    token_hash = hashlib.sha256(str(token).encode("utf-8")).hexdigest()
    used_at = now_iso()
    db = get_db()
    row = db.execute(
        prepare_sql(
            """
            UPDATE speed_training_attempts
            SET used_at = ?
            WHERE token_hash = ?
                AND user_id = ?
                AND used_at IS NULL
                AND expires_at > ?
            RETURNING token_hash
            """
        ),
        (used_at, token_hash, user_id, used_at),
    ).fetchone()
    db.commit()

    return row is not None


def leaderboard_display_name(value):
    parts = str(value or "Uczeń").strip().split()

    if not parts:
        return "Uczeń"

    if len(parts) == 1:
        return parts[0][:40]

    return f"{parts[0][:32]} {parts[-1][0].upper()}."


def speed_training_leaderboard(filters=None, limit=10, viewer_user_id=None):
    filters = filters or {}
    where = ["u.role = 'student'", "u.is_active = ?"]
    values = [db_bool(True)]
    filter_columns = {
        "level": "r.level",
        "topic": "r.topic",
        "difficulty": "r.difficulty",
        "round_seconds": "r.round_seconds",
    }

    for key, column in filter_columns.items():
        value = filters.get(key)

        if value in (None, ""):
            continue

        if key == "round_seconds":
            value = duration_seconds(value)

        where.append(f"{column} = ?")
        values.append(value)

    period = str(filters.get("period") or "").strip().lower()

    if period in {"week", "weekly"}:
        week_start = datetime.now(timezone.utc) - timedelta(days=7)

        where.append("r.created_at >= ?")
        values.append(week_start.isoformat())

    values.append(bounded_int(limit, 1, 50))
    rows = execute(
        f"""
        WITH ranked AS (
            SELECT
                r.id,
                r.user_id,
                u.display_name,
                u.level AS student_level,
                u.is_active AS student_is_active,
                r.level,
                r.topic,
                r.difficulty,
                r.round_seconds,
                r.correct_count,
                r.mistake_count,
                r.best_streak,
                r.average_time_seconds,
                r.created_at,
                ROW_NUMBER() OVER (
                    PARTITION BY r.user_id
                    ORDER BY
                        r.correct_count DESC,
                        r.mistake_count ASC,
                        r.average_time_seconds ASC,
                        r.best_streak DESC,
                        r.created_at DESC
                ) AS row_rank
            FROM speed_training_results r
            JOIN users u ON u.id = r.user_id
            WHERE {" AND ".join(where)}
        )
        SELECT *
        FROM ranked
        WHERE row_rank = 1
        ORDER BY
            correct_count DESC,
            mistake_count ASC,
            average_time_seconds ASC,
            best_streak DESC,
            created_at DESC
        LIMIT ?
        """,
        tuple(values),
    ).fetchall()

    leaderboard = []

    for index, row in enumerate(rows, start=1):
        item = speed_training_result_to_dict(row, viewer_user_id=viewer_user_id)
        item["display_name"] = leaderboard_display_name(item.get("display_name"))
        item["rank"] = index
        item.pop("row_rank", None)
        leaderboard.append(item)

    return leaderboard


def speed_training_history(user_id, filters=None, limit=12):
    filters = filters or {}
    where = ["user_id = ?"]
    values = [user_id]
    filter_columns = {
        "level": "level",
        "topic": "topic",
        "difficulty": "difficulty",
        "round_seconds": "round_seconds",
    }

    for key, column in filter_columns.items():
        value = filters.get(key)

        if value in (None, ""):
            continue

        if key == "round_seconds":
            value = duration_seconds(value)

        where.append(f"{column} = ?")
        values.append(value)

    period = str(filters.get("period") or "").strip().lower()

    if period in {"week", "weekly"}:
        week_start = datetime.now(timezone.utc) - timedelta(days=7)

        where.append("created_at >= ?")
        values.append(week_start.isoformat())

    values.append(bounded_int(limit, 1, 40))
    rows = execute(
        f"""
        SELECT *
        FROM (
            SELECT *
            FROM speed_training_results
            WHERE {" AND ".join(where)}
            ORDER BY created_at DESC
            LIMIT ?
        ) AS recent
        ORDER BY created_at ASC
        """,
        tuple(values),
    ).fetchall()

    return [speed_training_result_to_dict(row) for row in rows]


def mark_task_for_review(user_id, data):
    task_id = bounded_text(data.get("task_id"), 240)

    if not task_id:
        raise ValueError("Brakuje identyfikatora zadania.")

    existing = execute(
        """
        SELECT *
        FROM task_review_items
        WHERE user_id = ?
            AND task_id = ?
            AND is_resolved = ?
        ORDER BY created_at DESC
        LIMIT 1
        """,
        (user_id, task_id, db_bool(False)),
    ).fetchone()

    if existing:
        return review_item_to_dict(existing), False

    created_at = now_iso()
    db = get_db()
    values = (
        user_id,
        task_id,
        bounded_text(data.get("source_id"), 160),
        bounded_text(data.get("file"), 300),
        bounded_text(data.get("level"), 40),
        bounded_text(data.get("topic"), 160),
        bounded_int(data.get("difficulty"), 0, 10),
        bounded_text(data.get("title"), 240),
        bounded_text(data.get("course_part"), 80),
        bounded_text(data.get("note"), 1000),
        db_bool(False),
        created_at,
    )

    if database_engine() == "postgres":
        cursor = db.execute(
            prepare_sql(
                """
                INSERT INTO task_review_items (
                    user_id,
                    task_id,
                    source_id,
                    file,
                    level,
                    topic,
                    difficulty,
                    title,
                    course_part,
                    note,
                    is_resolved,
                    created_at
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                RETURNING *
                """
            ),
            values,
        )
        item = review_item_to_dict(cursor.fetchone())
    else:
        cursor = db.execute(
            """
            INSERT INTO task_review_items (
                user_id,
                task_id,
                source_id,
                file,
                level,
                topic,
                difficulty,
                title,
                course_part,
                note,
                is_resolved,
                created_at
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            values,
        )
        item = review_item_to_dict(
            db.execute(
                "SELECT * FROM task_review_items WHERE id = ?",
                (cursor.lastrowid,),
            ).fetchone()
        )

    db.commit()

    return item, True


def review_tasks_for_user(user_id, include_resolved=False):
    if include_resolved:
        rows = execute(
            """
            SELECT *
            FROM task_review_items
            WHERE user_id = ?
            ORDER BY is_resolved ASC, created_at DESC
            """,
            (user_id,),
        ).fetchall()
    else:
        rows = execute(
            """
            SELECT *
            FROM task_review_items
            WHERE user_id = ?
                AND is_resolved = ?
            ORDER BY created_at DESC
            """,
            (user_id, db_bool(False)),
        ).fetchall()

    return [review_item_to_dict(row) for row in rows]


def contact_message_to_dict(row):
    item = row_to_dict(row)

    if item:
        item["is_read"] = bool(item["is_read"])
        item["is_deleted"] = bool(item.get("deleted_at"))

    return item


def active_schedule_reservation_keys():
    return {
        str(row["term_key"])
        for row in execute("SELECT term_key FROM schedule_reservations").fetchall()
    }


def schedule_reservations_for_messages(message_ids):
    ids = []

    for value in message_ids:
        try:
            ids.append(int(value))
        except (TypeError, ValueError):
            continue

    if not ids:
        return {}

    unique_ids = list(dict.fromkeys(ids))
    placeholders = ", ".join("?" for _ in unique_ids)
    rows = execute(
        f"""
        SELECT message_id, term_label
        FROM schedule_reservations
        WHERE message_id IN ({placeholders})
        ORDER BY message_id ASC, created_at ASC
        """,
        tuple(unique_ids),
    ).fetchall()
    reservations = {message_id: [] for message_id in unique_ids}

    for row in rows:
        reservations.setdefault(int(row["message_id"]), []).append(str(row["term_label"]))

    return reservations


def release_schedule_reservations(message_id):
    db = get_db()
    message = execute(
        "SELECT id FROM contact_messages WHERE id = ?",
        (message_id,),
    ).fetchone()

    if not message:
        return None

    rows = execute(
        """
        SELECT term_label
        FROM schedule_reservations
        WHERE message_id = ?
        ORDER BY created_at ASC
        """,
        (message_id,),
    ).fetchall()
    db.execute(
        prepare_sql("DELETE FROM schedule_reservations WHERE message_id = ?"),
        (message_id,),
    )
    db.commit()

    return [str(row["term_label"]) for row in rows]


def create_contact_message(data, schedule_reservations=()):
    contact = str(data.get("contact") or "").strip()
    preferred_term = str(data.get("preferred_term") or "").strip()
    message = str(data.get("message") or "").strip()
    origin = str(data.get("origin") or "prospect").strip().lower()
    user_id = data.get("user_id")

    if origin == "parent" and not message:
        raise ValueError("Kontakt i treść wiadomości są wymagane.")

    if origin not in {"prospect", "parent"}:
        origin = "prospect"

    try:
        user_id = int(user_id) if user_id not in (None, "") else None
    except (TypeError, ValueError):
        user_id = None

    contact = contact[:254]
    preferred_term = preferred_term[:160] or None
    message = message[:3000]
    created_at = now_iso()
    db = get_db()
    values = (
        contact,
        preferred_term,
        message,
        origin,
        user_id,
        db_bool(False),
        created_at,
    )

    if database_engine() == "postgres":
        cursor = db.execute(
            prepare_sql(
                """
                INSERT INTO contact_messages (
                    contact,
                    preferred_term,
                    message,
                    origin,
                    user_id,
                    is_read,
                    created_at
                )
                VALUES (?, ?, ?, ?, ?, ?, ?)
                RETURNING *
                """
            ),
            values,
        )
        item = contact_message_to_dict(cursor.fetchone())
    else:
        cursor = db.execute(
            """
            INSERT INTO contact_messages (
                contact,
                preferred_term,
                message,
                origin,
                user_id,
                is_read,
                created_at
            )
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            values,
        )
        item = contact_message_to_dict(
            db.execute(
                "SELECT * FROM contact_messages WHERE id = ?",
                (cursor.lastrowid,),
            ).fetchone()
        )

    reservation_rows = []

    for reservation in schedule_reservations or ():
        if not isinstance(reservation, dict):
            raise ValueError("Nieprawidłowy termin rezerwacji.")

        term_key = str(reservation.get("key") or "").strip()
        term_label = str(reservation.get("label") or "").strip()

        if not term_key or not term_label or len(term_key) > 240 or len(term_label) > 160:
            raise ValueError("Nieprawidłowy termin rezerwacji.")

        reservation_rows.append((term_key, term_label))

    if len({row[0] for row in reservation_rows}) != len(reservation_rows):
        raise ValueError("Termin może zostać wybrany tylko raz.")

    try:
        for term_key, term_label in reservation_rows:
            db.execute(
                prepare_sql(
                    """
                    INSERT INTO schedule_reservations (
                        term_key,
                        message_id,
                        term_label,
                        created_at
                    )
                    VALUES (?, ?, ?, ?)
                    """
                ),
                (term_key, item["id"], term_label, created_at),
            )
    except INTEGRITY_ERRORS as exc:
        db.rollback()
        raise ScheduleReservationConflict from exc

    db.commit()

    return item


def list_contact_messages(limit=200, box="inbox", origin=None):
    show_trash = box == "trash"
    where = ["deleted_at IS NOT NULL" if show_trash else "deleted_at IS NULL"]
    values = []

    if origin in {"prospect", "parent"}:
        where.append("origin = ?")
        values.append(origin)

    order_clause = "deleted_at DESC, created_at DESC" if show_trash else "is_read ASC, created_at DESC"
    values.append(limit)

    messages = [
        contact_message_to_dict(row)
        for row in execute(
            f"""
            SELECT *
            FROM contact_messages
            WHERE {" AND ".join(where)}
            ORDER BY {order_clause}
            LIMIT ?
            """,
            tuple(values),
        )
        .fetchall()
    ]
    reservations = schedule_reservations_for_messages(
        message["id"] for message in messages
    )

    for message in messages:
        message["reserved_terms"] = reservations.get(message["id"], [])

    return messages


def contact_message_counts(origin=None):
    where = ""
    values = [db_bool(False)]

    if origin in {"prospect", "parent"}:
        where = "WHERE origin = ?"
        values.append(origin)

    row = execute(
        f"""
        SELECT
            SUM(CASE WHEN deleted_at IS NULL THEN 1 ELSE 0 END) AS inbox,
            SUM(CASE WHEN deleted_at IS NOT NULL THEN 1 ELSE 0 END) AS trash,
            SUM(CASE WHEN deleted_at IS NULL AND is_read = ? THEN 1 ELSE 0 END) AS unread
        FROM contact_messages
        {where}
        """,
        tuple(values),
    ).fetchone()

    if row is None:
        return {
            "inbox": 0,
            "trash": 0,
            "unread": 0,
        }

    return {
        "inbox": int(row["inbox"] or 0),
        "trash": int(row["trash"] or 0),
        "unread": int(row["unread"] or 0),
    }


def update_contact_message_read_state(message_id, is_read=True):
    read_at = now_iso() if is_read else None
    db = get_db()

    if database_engine() == "postgres":
        cursor = db.execute(
            prepare_sql(
                """
                UPDATE contact_messages
                SET is_read = ?, read_at = ?
                WHERE id = ?
                RETURNING *
                """
            ),
            (db_bool(is_read), read_at, message_id),
        )
        item = contact_message_to_dict(cursor.fetchone())
    else:
        cursor = db.execute(
            """
            UPDATE contact_messages
            SET is_read = ?, read_at = ?
            WHERE id = ?
            """,
            (db_bool(is_read), read_at, message_id),
        )

        if cursor.rowcount == 0:
            item = None
        else:
            item = contact_message_to_dict(
                db.execute(
                    "SELECT * FROM contact_messages WHERE id = ?",
                    (message_id,),
                ).fetchone()
            )

    db.commit()

    return item


def update_contact_message_deleted_state(message_id, is_deleted=True):
    deleted_at = now_iso() if is_deleted else None
    db = get_db()

    if database_engine() == "postgres":
        cursor = db.execute(
            prepare_sql(
                """
                UPDATE contact_messages
                SET deleted_at = ?
                WHERE id = ?
                RETURNING *
                """
            ),
            (deleted_at, message_id),
        )
        item = contact_message_to_dict(cursor.fetchone())
    else:
        cursor = db.execute(
            """
            UPDATE contact_messages
            SET deleted_at = ?
            WHERE id = ?
            """,
            (deleted_at, message_id),
        )

        if cursor.rowcount == 0:
            item = None
        else:
            item = contact_message_to_dict(
                db.execute(
                    "SELECT * FROM contact_messages WHERE id = ?",
                    (message_id,),
                ).fetchone()
            )

    db.commit()

    return item


def delete_contact_message(message_id):
    db = get_db()
    item = contact_message_to_dict(
        execute(
            "SELECT * FROM contact_messages WHERE id = ?",
            (message_id,),
        ).fetchone()
    )

    if not item:
        return None

    db.execute(
        prepare_sql("DELETE FROM schedule_reservations WHERE message_id = ?"),
        (message_id,),
    )
    db.execute(
        prepare_sql("DELETE FROM contact_messages WHERE id = ?"),
        (message_id,),
    )
    db.commit()

    return item


def update_review_task_resolution(item_id, is_resolved=True):
    resolved_at = now_iso() if is_resolved else None
    db = get_db()

    if database_engine() == "postgres":
        cursor = db.execute(
            prepare_sql(
                """
                UPDATE task_review_items
                SET is_resolved = ?, resolved_at = ?
                WHERE id = ?
                RETURNING *
                """
            ),
            (db_bool(is_resolved), resolved_at, item_id),
        )
        item = review_item_to_dict(cursor.fetchone())
    else:
        cursor = db.execute(
            """
            UPDATE task_review_items
            SET is_resolved = ?, resolved_at = ?
            WHERE id = ?
            """,
            (db_bool(is_resolved), resolved_at, item_id),
        )

        if cursor.rowcount == 0:
            item = None
        else:
            item = review_item_to_dict(
                db.execute(
                    "SELECT * FROM task_review_items WHERE id = ?",
                    (item_id,),
                ).fetchone()
            )

    db.commit()

    return item
