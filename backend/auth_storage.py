...
PARENT_ACCESS_MAX_AGE_DAYS = max(
    1,
    min(int(os.environ.get("PARENT_ACCESS_MAX_AGE_DAYS", "90")), 365),
)
...
def init_auth_db():
    ...
    default_parent_expiry = (
        datetime.now(timezone.utc) + timedelta(days=PARENT_ACCESS_MAX_AGE_DAYS)
    ).isoformat()
    db.execute(
        prepare_sql(
            "UPDATE parent_access_tokens SET expires_at = ? WHERE expires_at IS NULL"
        ),
        (default_parent_expiry,),
    )
